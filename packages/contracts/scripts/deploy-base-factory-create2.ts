...
async function deploySupportDeployer(
  name: "LaunchTokenDeployer" | "LaunchCreate2Deployer",
...
async function main() {
  ...
  const whitelistDeployerContract = await ethers.getContractAt(
    "LaunchCreate2Deployer",
    whitelistDeployer.address
  );
  const whitelistDeployerFactory = await whitelistDeployerContract.factory();
  if (whitelistDeployerFactory === ethers.ZeroAddress) {
    const bindTx = await whitelistDeployerContract.setFactory(vanity.predicted);
    console.log(`Broadcasted whitelist deployer bind tx: ${bindTx.hash}`);
    const bindReceipt = await bindTx.wait();
    if (!bindReceipt || bindReceipt.status !== 1) {
      throw new Error("Failed to bind whitelistDeployer to factory");
    }
  } else if (whitelistDeployerFactory.toLowerCase() !== vanity.predicted.toLowerCase()) {
    throw new Error(
      `whitelistDeployer already bound to ${whitelistDeployerFactory}, expected ${vanity.predicted}`
    );
  }
  ...
  const taxedDeployerContract = await ethers.getContractAt(
    "LaunchCreate2Deployer",
    taxedDeployer.address
  );
  const taxedDeployerFactory = await taxedDeployerContract.factory();
  if (taxedDeployerFactory === ethers.ZeroAddress) {
    const bindTx = await taxedDeployerContract.setFactory(vanity.predicted);
    console.log(`Broadcasted taxed deployer bind tx: ${bindTx.hash}`);
    const bindReceipt = await bindTx.wait();
    if (!bindReceipt || bindReceipt.status !== 1) {
      throw new Error("Failed to bind taxedDeployer to factory");
    }
  } else if (taxedDeployerFactory.toLowerCase() !== vanity.predicted.toLowerCase()) {
    throw new Error(
      `taxedDeployer already bound to ${taxedDeployerFactory}, expected ${vanity.predicted}`
    );
  }
