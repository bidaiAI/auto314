...
describe("LaunchTokenWhitelist", function () {
  ...
  it("lets claimed whitelist holders auto-sell by transferring tokens back to the contract", async function () {
    ...
    const tx = await token.connect(buyer).transfer(tokenAddr, sellAmount);
  ...
  it("keeps fee vaults and accounted balances exact when whitelist finalizes", async function () {
    ...
    expect((await token.protocolFeeVault()) + (await token.graduationAssistReserve())).to.equal(protocolFee);
