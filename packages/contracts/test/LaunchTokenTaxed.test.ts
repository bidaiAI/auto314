...
describe("LaunchTokenTaxed", function () {
  ...
  it("rejects secondary pools created by a foreign dex factory", async function () {
    const { buyer, creator, token } = await deployFixture();
  ...
  it("marks sell-side transfer history without mislabeling taxed pool buys as transfer-outs", async function () {
    const { buyer, other, token, pair } = await deployFixture();

    await token.connect(buyer).buy(0, { value: OVERBUY });

    const buyerBalance = await token.balanceOf(buyer.address);
    const seededSellAmount = buyerBalance / 10n;
    await token.connect(buyer).transfer(await pair.getAddress(), seededSellAmount);
    expect(await token.hasTransferredOut(buyer.address)).to.equal(true);
    ...
    const grossBuyAmount = seededSellAmount / 2n;
    await token.connect(pairSigner).transfer(other.address, grossBuyAmount);

    expect(await token.hasTransferredOut(other.address)).to.equal(false);

    const otherBalance = await token.balanceOf(other.address);
    expect(otherBalance).to.be.gt(0n);

    await token.connect(other).transfer(await pair.getAddress(), otherBalance / 2n);
    expect(await token.hasTransferredOut(other.address)).to.equal(true);

    await network.provider.request({ method: "hardhat_stopImpersonatingAccount", params: [pairAddress] });
  });

  it("auto-sells on transfer-to-contract before graduation without applying tax", async function () {
    const { buyer, treasury, token } = await deployFixture();
    const dead = "0x000000000000000000000000000000000000dEaD";

    await token.connect(buyer).buy(0, { value: ethers.parseEther("0.05") });
    ...
    await expect(token.connect(buyer).transfer(await token.getAddress(), sellAmount))
      .to.emit(token, "SellExecuted")
      .withArgs(buyer.address, sellAmount, anyValue, preview[1], anyValue, anyValue, anyValue, anyValue);
