...
- **尾盘处理**：新工厂会先从协议费里预留最多 `0.005 native` 作为 graduation assist reserve；当剩余毕业缺口进入最后 `0.005 native` 且储备充足时，会冻结普通买入/卖出/转账，只保留 `pokeGraduation()` 完成毕业。在此之前，`Bonding314` 阶段支持把代币直接转到 token 合约地址触发卖出，但仍禁止普通钱包互转，直到进入 `DEXOnly`
