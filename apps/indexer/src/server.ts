...
const metadataUploadEnabled = process.env.INDEXER_ENABLE_METADATA_UPLOAD === "1";
const metadataMaxBytesRaw = Number(process.env.INDEXER_METADATA_MAX_BYTES ?? 65_536);
const metadataMaxBytes = Number.isFinite(metadataMaxBytesRaw) && metadataMaxBytesRaw > 0
  ? Math.floor(metadataMaxBytesRaw)
  : 65_536;
const metadataIdPattern = /^[a-z0-9-]{6,64}$/;
...
function sendJson(res: ServerResponse, status: number, body: unknown) {
  ...
  res.setHeader("access-control-allow-methods", metadataUploadEnabled ? "GET, POST, OPTIONS" : "GET, OPTIONS");
...
function isSafeMetadataId(value: string) {
  return metadataIdPattern.test(value);
}
...
function readBody(req: IncomingMessage, maxBytes = metadataMaxBytes): Promise<string> {
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (req.method === "POST" && url.pathname === "/api/metadata") {
      if (!metadataUploadEnabled) {
        sendJson(res, 403, { error: "Reference metadata upload is disabled in this deployment." });
        return;
      }
      ...
      try {
        ...
        if (error instanceof RequestBodyTooLargeError) {
          sendJson(res, 413, { error: "Metadata body too large", limitBytes: error.maxBytes });
          return;
        }
        throw error;
      }
      ...
      try {
        const metadata = JSON.parse(body) as { name?: unknown; symbol?: unknown };
        if (
    ...
    if (req.method === "GET" && url.pathname.startsWith("/api/metadata/")) {
      ...
      if (!isSafeMetadataId(id)) {
        sendJson(res, 400, { error: "Invalid metadata id" });
        return;
      }
